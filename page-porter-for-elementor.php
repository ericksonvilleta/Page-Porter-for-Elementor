...
class Page_Porter_For_Elementor {
	...
	public function render_admin_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Page Porter for Elementor', 'page-porter-elementor' ); ?></h1>
			<p><?php echo esc_html__( 'Bulk move your Elementor pages with ease. Author: ', 'page-porter-elementor' ) . '<strong>Erick Villeta</strong>'; ?></p>
			<hr>
			<div style="display: flex; gap: 40px; margin-top: 20px;">
				<div style="flex: 1; background: #fff; padding: 20px; border: 1px solid #ccd0d4;">
					<h2><?php esc_html_e( 'Export Pages', 'page-porter-elementor' ); ?></h2>
					<form method="post">
						...
						<label><input type="checkbox" name="export_all"> <?php esc_html_e( 'Export All Pages', 'page-porter-elementor' ); ?></label>
						<?php submit_button( __( 'Generate Export File', 'page-porter-elementor' ) ); ?>
					</form>
				</div>

				<div style="flex: 1; background: #fff; padding: 20px; border: 1px solid #ccd0d4;">
					<h2><?php esc_html_e( 'Import Pages', 'page-porter-elementor' ); ?></h2>
					<form method="post" enctype="multipart/form-data">
						<?php wp_nonce_field( 'porter_import_nonce', 'porter_nonce' ); ?>
						<input type="hidden" name="action" value="porter_import">
						<input type="file" name="import_file" accept=".json" required>
						<p style="font-size: 12px; color: #666;"><?php esc_html_e( 'The importer will use existing media files if found to avoid duplicates.', 'page-porter-elementor' ); ?></p>
						<?php submit_button( __( 'Run Smart Import', 'page-porter-elementor' ) ); ?>
					</form>
				</div>
			</div>
		</div>
	...
	public function handle_import() {
		if ( isset( $_POST['action'] ) && 'porter_import' === $_POST['action'] && ! empty( $_FILES['import_file']['tmp_name'] ) ) {
			...
			$import_file = $_FILES['import_file']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			$file_path   = sanitize_text_field( wp_unslash( $import_file['tmp_name'] ) );
			...
			if ( ! isset( $decoded_data['pages'] ) || ! is_array( $decoded_data['pages'] ) ) {
				add_settings_error( 'porter_msg', 'porter_error', 'Invalid export file.', 'error' );
				return;
			}
			...
			add_settings_error( 'porter_msg', 'porter_success', 'Import successful! Pages are in Drafts.', 'updated' );
		}
	}
	...
	private function get_attachment_id_by_url( $url ) {
		...
		$cache_key     = 'porter_attachment_id_' . md5( $file_name );
